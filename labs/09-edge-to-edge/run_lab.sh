#!/usr/bin/env bash
# Lab 09: Edge-to-Edge Tactical Red-Teaming Runner
...
if ! curl -s "http://$TARGET_HOST:$TARGET_PORT/health" >/dev/null 2>&1; then
    ...
    python3 "$SCRIPT_DIR/../04-local-llm-target/atlas_target.py" > /tmp/atlas_lab09.log 2>&1 &
    ATLAS_PID=$!
    trap 'kill $ATLAS_PID 2>/dev/null || true' EXIT
    sleep 2
fi

echo ">>> Launching Lab 09 Edge-to-Edge Tactical Red-Teaming..."
python3 nano_edge_probe.py
